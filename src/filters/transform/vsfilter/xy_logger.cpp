...
void XyDisplayType(LPTSTR label, const AM_MEDIA_TYPE *pmtIn)
{
    ...
    if (pmtIn->formattype == FORMAT_VideoInfo) {
        ...
        XY_LOG_TRACE(_T("Image size      £º")<<pbmi->biSizeImage    );
        XY_LOG_TRACE(_T("Planes          £º")<<pbmi->biPlanes       );
        XY_LOG_TRACE(_T("X Pels per metre£º")<<pbmi->biXPelsPerMeter);
        XY_LOG_TRACE(_T("Y Pels per metre£º")<<pbmi->biYPelsPerMeter);
        XY_LOG_TRACE(_T("Colours used    £º")<<pbmi->biClrUsed      );
