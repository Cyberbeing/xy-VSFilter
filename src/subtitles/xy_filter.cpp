...
void xy_be_blur(PUINT8 src, int width, int height, int stride, float pass_x, float pass_y)
{
    ...
    //filter_bias_375 uses a rounding with bias -.375
    //filter_bias_125 uses a rounding with bias -.125
    XyBeFilter filter_bias_375 = (g_cpuid.m_flags & CCpuID::sse2) ? xy_be_filter_sse<ROUND_DOWN> : xy_be_filter_c<ROUND_DOWN>;
    XyBeFilter filter_bias_125 = (g_cpuid.m_flags & CCpuID::sse2) ? xy_be_filter_sse<ROUND_HALF_DOWN> : xy_be_filter_c<ROUND_HALF_DOWN>;
