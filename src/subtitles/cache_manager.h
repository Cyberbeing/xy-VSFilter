...
class CWordPaintResultKey
{
public:
    CWordPaintResultKey(const SharedPtrCWord& word, 
        const CPoint& shadow_pos, const CPoint& outline_pos, const CPoint& body_pos, 
        const CPoint& trans_org) 
        : m_word(word)
        , m_shadow_pos(shadow_pos), m_outline_pos(outline_pos), m_body_pos(body_pos)
        , m_trans_org(trans_org){}
    bool operator==(const CWordPaintResultKey& key)const;
...
public:
    ULONG m_hash_value;
private:
    typedef ::boost::shared_ptr<CWordPaintResultKey> SharedWordKey;
    SharedWordKey m_overlay_key;
