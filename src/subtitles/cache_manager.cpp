...
bool CWordPaintResultKey::operator==( const CWordPaintResultKey& key ) const
{
    return (this==&key) || (
        *m_word == *key.m_word &&
        m_shadow_pos == key.m_shadow_pos &&
        m_outline_pos == key.m_outline_pos &&
        m_body_pos == key.m_body_pos &&
        m_trans_org == key.m_trans_org);
}
